import React from "react";
import { slide as Menu } from "react-burger-menu";
import "./Sidebar.css";
import { FaHome, FaCalendarAlt, FaUsers, FaHistory } from "react-icons/fa";

const SideBar = () => {
  return (
    <>    
    <Menu right>
      <div style = {{background: "none", border: "none", borderBottom:"2px solid white", fontWeight: "bold", fontSize: "30px", padding:"0px", position:"relative", top:"5%",paddingBottom:"15%"}}>Polyplace</div>
      <a className="menu-item" href="/">
        <FaHome style = {{color:"white", paddingRight:"3px"}} />
        Home
      </a>
      <a className="menu-item" href="/rooms">
        <FaUsers style = {{color:"white",paddingRight:"3px"}}/>
        Rooms
      </a>
      <a className="menu-item" href="/history">
        <FaHistory style = {{color:"white",paddingRight:"3px"}}/>
        History
      </a>
      <a className="menu-item" href="/buildings">
        <FaCalendarAlt style = {{color:"white",paddingRight:"3px"}}/>
        Buildings
      </a>
    </Menu>
    </>

  );
};

export default SideBar;
